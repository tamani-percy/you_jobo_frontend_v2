...
const {updateJobApplicationInterviewMutation, updateJobApplicationInterview} = useInterviews()
...
const state = reactive({
  interviewerName: props.interview.interviewerName ?? '',
  interviewerEmail: props.interview.interviewerEmail ?? '',
  locationOrLink: props.interview.locationOrLink ?? '',
  durationInMinutes: props.interview.durationMinutes as number | undefined,

  interviewDate: props.interview.scheduledAt
      ? parseDate(props.interview.scheduledAt.slice(0, 10))
      : undefined as CalendarDate | undefined,

  interviewStage: props.interview.interviewStage ?? '',
  interviewType: props.interview.interviewType ?? '',
  interviewResult: props.interview.interviewResult ?? '',
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  updateJobApplicationInterview({
    ...
    jobApplicationId: props.interview.jobApplicationId,
    //@ts-ignore
...
<template>
  <UModal v-model:open="open" title="Add interview" :ui="{ content: 'sm:max-w-xl md:min-w-6xl' }">
    <UButton icon="i-lucide-pencil" variant="subtle" color="neutral"/>
    <template #body>
      <!-- @vue-ignore -->
      <UForm
          ref="form"
      ...
      >
        ...
        <div class="flex items-center justify-between pt-4">
          ...
          <div>
            ...
            <UButton
                :loading="updateJobApplicationInterviewMutation.isLoading.value"
                v-else
                type="submit"
            >
              Submit
